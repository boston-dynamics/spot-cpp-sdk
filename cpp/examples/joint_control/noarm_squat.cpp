...
::bosdyn::common::Status run(const ::bosdyn::client::CommonCLIArgs& args) {
    ...
    JointAPIInterface<SpotKin::N_DOF_LEGS> joint_api_interface;
    ...
    std::thread state_thread(&JointAPIInterface<SpotKin::N_DOF_LEGS>::handle_state_streaming,
                             &joint_api_interface, robot_state_stream_client);

    // Async activate once streaming has started
    std::thread activate_thread(&JointAPIInterface<SpotKin::N_DOF_LEGS>::activate,
                                &joint_api_interface, robot_command_client);
    ...
    std::vector<std::array<float, SpotKin::N_DOF_LEGS>> cmd_poses;
    std::array<float, SpotKin::N_DOF_LEGS> squatty_pose = {0.15, 1.3, -2.25, -0.15, 1.3, -2.25,
                                                           0.15, 1.3, -2.25, -0.15, 1.3, -2.25};
    cmd_poses.push_back(curr_pose);
    cmd_poses.push_back(squatty_pose);
    cmd_poses.push_back(curr_pose);
    cmd_poses.push_back(squatty_pose);
    cmd_poses.push_back(curr_pose);
    ...
    int64_t duration_bw_pose_ns = 2 * 1e9;
    auto leg_k_q_p = spot_kin.gains.k_q_p_leg;
    auto leg_k_qd_p = spot_kin.gains.k_qd_p_leg;
    ...
    status = joint_api_interface.generate_joint_pos_interp_commands(
        ...
        duration_bw_pose_ns, leg_k_q_p, leg_k_qd_p);
    if (!status) {
        return status;
    }
...
int main(int argc, char** argv) {
    // Parse the arguments.
    CLI::App parser{"NoArmSquatExample"};
    ...
    std::cout << "------NoArmSquatExample application worked correctly." << std::endl;
    return 0;
}
