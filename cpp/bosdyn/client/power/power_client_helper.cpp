...
::bosdyn::common::Status SafePowerOffMotors(RobotCommandClient* robot_command_client,
                          RobotStateClient* robot_state_client, ::bosdyn::common::Duration timeout,
                          double update_frequency) {
    // Make sure the provided arguments are valid.
    // Robot command client and robot state client are required.
    BOSDYN_ASSERT_PRECONDITION(robot_command_client != nullptr, "Robot command client must not be null for SafePowerOff.");
    BOSDYN_ASSERT_PRECONDITION(robot_state_client != nullptr, "Robot state client must not be null for SafePowerOff.");
    ...
    BOSDYN_ASSERT_PRECONDITION(update_frequency != 0.0, "Update frequency for SafePowerOff feedback cannot be 0.");
...
::bosdyn::common::Status PowerOnMotors(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                     double update_frequency) {
...
::bosdyn::common::Status PowerOffMotors(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                      double update_frequency) {
    ::bosdyn::api::PowerCommandRequest command_request;
...
::bosdyn::common::Status SafePowerOffRobot(RobotCommandClient* robot_command_client,
                         RobotStateClient* robot_state_client, PowerClient* power_client,
    ...
    auto safe_power_off_status = SafePowerOffMotors(robot_command_client, robot_state_client,
                                                    end_time - ::bosdyn::common::NsecSinceEpoch(),
                                                    update_frequency);
...
::bosdyn::common::Status PowerCycleRobot(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                       double update_frequency) {
    ::bosdyn::api::PowerCommandRequest command_request;
...
::bosdyn::common::Status PowerOnPayloadPorts(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                           double update_frequency) {
    ::bosdyn::api::PowerCommandRequest command_request;
...
::bosdyn::common::Status PowerOffPayloadPorts(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                            double update_frequency) {
    ::bosdyn::api::PowerCommandRequest command_request;
...
::bosdyn::common::Status PowerOnWifiRadio(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                    double update_frequency) {
    ::bosdyn::api::PowerCommandRequest command_request;
...
::bosdyn::common::Status PowerOffWifiRadio(PowerClient* power_client, ::bosdyn::common::Duration timeout,
                     double update_frequency) {
    ::bosdyn::api::PowerCommandRequest command_request;
...
::bosdyn::common::Status PowerCommand(PowerClient* power_client,
                    ::bosdyn::api::PowerCommandRequest& request, ::bosdyn::common::Duration timeout,
                    double update_frequency) {
    // The update frequency cannot be 0 because we use it to compute time.
    // NOTE: We are not erroring on negative update_frequency, it triggers no sleep between tries.
    BOSDYN_ASSERT_PRECONDITION(update_frequency != 0.0,  "Update frequency for PowerCommand feedback cannot be 0.");
    ...
    while (::bosdyn::common::NsecToSec(::bosdyn::common::NowNsec()) < end_time) {
        ...
        if (status == std::future_status::ready) {
            ...
            if (feedback.response.status() != ::bosdyn::api::PowerCommandStatus::STATUS_IN_PROGRESS) {
...
Result<bool> IsPoweredOn(RobotStateClient* robot_state_client) {
    ...
    BOSDYN_ASSERT_PRECONDITION(robot_state_client != nullptr, "Robot state client cannot be null for IsPoweredOn.");
    ...
    if (result.response.robot_state().power_state().motor_power_state() == ::bosdyn::api::PowerState::STATE_ON) {
